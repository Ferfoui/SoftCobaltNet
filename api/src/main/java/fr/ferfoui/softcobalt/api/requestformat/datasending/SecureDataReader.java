 ...
 * The decryption is done using the {@link EncryptDecryptCipher#decrypt(byte[],
 * Key, String)} method.
 */
@Deprecated
public class SecureDataReader implements SecureExchanger {
  ...
  /**
   * Reads the data.
   * If the decryption is enabled, the data is decrypted using the key and the
  ...
  public DataReader readData(byte[] data)
    ...
    if (isDecryptionEnabled) {
      byte[] decryptedData =
          EncryptDecryptCipher.decrypt(data, key, keyAlgorithm);
   ...
   * The decryption is disabled by default, and the key must be set before
   * enabling it.
   *
