 ...
 * <p>
 * This class is used to handle the client socket connection and process the
 * requests. It is used by the ServerSocketManager to handle the client
 * connections.
 *
 * @author Ferfoui
 * @since 1.0
...
public class ClientSocketHandler
    extends DataQueueSocketManager implements Runnable {
   ...
   * @param logger           The logger to use, if null, a new logger will be
   *     created
   */
  public ClientSocketHandler(Socket socket, RequestProcessor clientConnection,
                             long clientId, @Nullable Logger logger) {
    super(socket, logger);
  ...
  protected Logger getLogger(@Nullable Logger logger) {
    return (logger == null)
        ? LoggerFactory.getLogger("client-socket-handler-" + clientId)
        : LoggerFactory.getLogger(logger.getName() + "-"
                                  + "client-socket-handler-" + clientId);
  }
  ...
  public void run() {
    ...
    try {
      ...
      while (doContinueListening) {
        ...
        logger.debug("The server should continue listening: {}",
                     doContinueListening);
      }
   ...
   */
  public void stopHandler() { doContinueListening = false; }
}
