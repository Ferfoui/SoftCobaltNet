...
public class ServerSocketManager {
  ...
  private final ExecutorService clientHandlersExecutor =
      Executors.newCachedThreadPool(new ServerThreadFactory());
   ...
   * @param clientConnectionProvider The client connection provider to use. This
   *     is used to create a new client connection
   */
  public ServerSocketManager(
      String name, ClientConnectionProvider clientConnectionProvider) {
    ...
    while (doContinueAccept) {
      try {
        ...
        ClientSocketHandler clientSocketHandler = new ClientSocketHandler(
            connectionSocket,
            getNewClientConnection(connectionSocket, finalClientId),
            finalClientId, null);

        clientHandlersExecutor.submit(clientSocketHandler);
        clientHandlersExecutor.submit(
            clientSocketHandler.getQueueHandlerRunnable());
